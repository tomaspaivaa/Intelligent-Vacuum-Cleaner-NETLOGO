...
aspiradores-own[energia capacidadeMaxCarga nLixo isWaitingCarregamento isWaitingLixo ticksEsperaCarregamento ticksEsperaLixo localXCarregamento localYCarregamento localXLixo localYLixo isTerminado]
...
to go
  ...
  if all? turtles [isTerminado = true] [ ;; Condição de final de experiência, quando o lixo tiver todo sido apanhado e despejado, a experiência acaba
...
to setup-patches
  ...
  ask patches with [not (pxcor = 16) and
                 ...
                 not (pycor = -16) and
                 not (pycor = 16)] [
    if random 101 <= percentagemVermelho [
      set pcolor red
    ]
  ]
  ask n-of nPatchesAzul patches with [pcolor = black] [
    set pcolor blue
  ]
  ask n-of nPatchesBranco patches with [pcolor = black] [
    set pcolor white
  ]
...
to setup-turtles
  ...
  create-aspiradores naspiradores[
    ...
    set nLixo 0
    set heading 180
    set isWaitingCarregamento false
    set isWaitingLixo false
    set isTerminado false
    set energia quantEnergia
    set ticksEsperaCarregamento tempoTicksCarregamento
    set ticksEsperaLixo tempoTicksDespejarLixo
    set capacidadeMaxCarga capCarga
    set localXCarregamento 1000
    set localYCarregamento 1000
    RandomSpawn
  ]
end
...
to StopWaitingCarregamento
  ifelse ticksEsperaCarregamento <= 0[
    set isWaitingCarregamento false
    set ticksEsperaCarregamento tempoTicksCarregamento
  ]
  [
    set ticksEsperaCarregamento ticksEsperaCarregamento - 1
  ]
end

to StopWaitingLixo
  ifelse ticksEsperaLixo <= 0[
    set isWaitingLixo false
    set ticksEsperaLixo tempoTicksDespejarLixo
  ]
  [
    set ticksEsperaLixo ticksEsperaLixo - 1
  ]
end

to MemorizaCarregador [coordX coordY]
  if localXCarregamento = 1000 and localYCarregamento = 1000 [
    set localXCarregamento coordX
    set localYCarregamento coordY
  ]
end

to AheadCarregar
  ifelse abs(xcor - localXCarregamento) >= 0.5[
    ifelse xcor < localXCarregamento[
  ...
  [if abs(ycor - localYCarregamento) >= 0.5
    [ifelse ycor < localYCarregamento[
      set heading 0
      set energia energia - 1
      ]
      [
...
to InfoNeighbour
  ask aspiradores [
    if localXCarregamento != 1000 and localYCarregamento != 1000 [
      ...
      if any? neighbours [
        ask neighbours [
          set localXCarregamento localX1
          set localYCarregamento localY1
        ]
      ]
...
to MoveAspiradores
  ask aspiradores[
    ifelse isWaitingCarregamento = true[
      StopWaitingCarregamento
    ]
    [
      ifelse isWaitingLixo = true[
        StopWaitingLixo
      ]
      [
        ifelse energia <= 0[
        ...
        [ InfoNeighbour
          ...
          [
            ifelse energia < poupancaEnergia [
              ...
              [
                ...
                [ifelse localXCarregamento = 1000 and localYCarregamento = 1000[
                  AheadGeral
                  ]
                  [
                    AheadCarregar
            ...
            [
              ...
              [ifelse nLixo >=  capacidadeMaxCarga[
...
@#$#@#$#@
<experiments>
  <experiment name="Base_Poup" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="Normal_base" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
  ...
  <experiment name="Base_nAspiradores10_comPoupanca_Energia65" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>count patches with [pcolor = red]</metric>
  ...
  <experiment name="Base_nAspiradores70_semPoupanca_Energia65" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>count patches with [pcolor = red]</metric>
  ...
  <experiment name="Base_Energia75_CapCarga10" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>count patches with [pcolor = red]</metric>
